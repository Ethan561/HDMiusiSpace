...
enum HD_LY_API {
    ...
    //获取博物馆列表
    case guideMuseumList(city_name: String, longitude: String, latitude: String, type:Int, skip:Int, take:Int, api_token: String)
    ...
    //展览博物馆分类
    case exhibitionNewMuseumList(skip:Int, take:Int, city_name: String, longitude: String, latitude: String,  api_token: String)
    ...
    //判断是否购买
    case orderIsBuyCheck(api_token: String, id:Int)
...
//            let dic2 = ["Sign": signKey]
        ...
        case .guideMuseumList(let city_name, let longitude, let latitude, let type, let skip, let take, let api_token):

            params = params.merging(["city_name": city_name, "longitude": longitude, "latitude": latitude, "type": type, "skip": skip, "take": take, "api_token": api_token], uniquingKeysWith: {$1})
            let signKey =  HDDeclare.getSignKey(params)
            let dic2 = ["Sign": signKey]
            params.merge(dic2, uniquingKeysWith: { $1 })
        ...
        case .exhibitionNewMuseumList(let skip, let take, let city_name, let longitude, let latitude, let api_token):
            params = params.merging(["skip": skip, "take": take, "city_name": city_name, "longitude": longitude, "latitude": latitude, "api_token": api_token], uniquingKeysWith: {$1})
            let signKey =  HDDeclare.getSignKey(params)
            let dic2 = ["Sign": signKey]
            params.merge(dic2, uniquingKeysWith: { $1 })
        
        case .orderIsBuyCheck(let api_token, let id):
            params = params.merging(["api_token": api_token, "id": id], uniquingKeysWith: {$1})
            let signKey =  HDDeclare.getSignKey(params)
            let dic2 = ["Sign": signKey]
            params.merge(dic2, uniquingKeysWith: { $1 })
