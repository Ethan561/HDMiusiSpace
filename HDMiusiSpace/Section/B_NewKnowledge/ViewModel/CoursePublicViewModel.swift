...
class CoursePublicViewModel: NSObject {
   ...
   class func doLikeRequest( id: String, cate_id: String, vc: UIViewController, success: @escaping((Data) -> Void), failure: ((Int?, String) ->Void)? )  {
    ...
    func reportCommentContent(api_token: String, option_id_str: String, comment_id: Int,content:String)  {
        HD_LY_NetHelper.loadData(API: HD_LY_API.self, target: .sendError(api_token: api_token, option_id_str: option_id_str, parent_id: String(comment_id), cate_id: "5", content: content, uoload_img: [""]), success: { (result) in
            let dic = HD_LY_NetHelper.dataToDictionary(data: result)
            LOG("\(String(describing: dic))")
