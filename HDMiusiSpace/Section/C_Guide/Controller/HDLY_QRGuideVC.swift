...
class HDLY_QRGuideVC: HDItemBaseVC {
    ...
    func showWebVC(url: String) {
        if url.contains("http") {
            ...
            let alert:UIAlertController = UIAlertController.init(title: "", message: "二维码无效，请重新扫描", preferredStyle: UIAlertController.Style.alert)
            let cancle = UIAlertAction.init(title: "确定", style: UIAlertAction.Style.cancel) { (action) in
                self.codeView.rerunning()
            }
