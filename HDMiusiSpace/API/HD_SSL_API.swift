...
enum HD_SSL_API {
    ...
    //获取攻略详情数据
    case getStrategyData(api_token: String,strategyId: Int)
}
...
extension HD_SSL_API: TargetType {
    ...
    var path: String {
        ...
        case .getStrategyData(api_token: _, strategyId: _):
            return "/api/exhibition/strategy_info"
        //...
...
//            let signKey =  HDDeclare.getSignKey(params)
        ...
        case .requestMyOrderList(api_token: let api_token, status: let status, skip: let skip, take: let take):
            params = params.merging(["status":status,"skip":skip,"take":take,"api_token":api_token], uniquingKeysWith: {$1})
            
            let signKey =  HDDeclare.getSignKey(params)
            let dic2 = ["Sign": signKey]
            params.merge(dic2, uniquingKeysWith: { $1 })
            
        case .requestMyOrderDetail(api_token: let api_token, orderId: let orderId):
        ...
        case .getMyOrderShareData(api_token: let api_token, orderId: let orderId):
            ...
            let dic2 = ["Sign": signKey]
            params.merge(dic2, uniquingKeysWith: { $1 })
        case .getStrategyData(api_token: let api_token, strategyId: let strategyId):
            params = params.merging(["strategy_id":strategyId,"api_token":api_token], uniquingKeysWith: {$1})
